...
def crop_volume_deskew(original_volume:Union[da.core.Array,np.ndarray,cle._tier0._pycl.OCLArray,resource_backed_dask_array.ResourceBackedDaskArray], 
                        deskewed_volume:Union[da.core.Array,np.ndarray,cle._tier0._pycl.OCLArray,resource_backed_dask_array.ResourceBackedDaskArray]=None, 
    ...
    reverse_aff, excess_bounds, deskew_transform = get_inverse_affine_transform(original_volume,angle_in_degrees,voxel_size_x,voxel_size_y,voxel_size_z,skew_dir)
    ...
    if deconvolution:
        if decon_processing == "cuda_gpu":
            crop_volume_processed = pycuda_decon(image = crop_volume, 
                                   psf = psf,
                                   dzdata=voxel_size_z,
                                   dxdata=voxel_size_x,
                                   dzpsf=voxel_size_z,
        ...
        if get_deskew_and_decon:
            deskewed_no_decon = cle.affine_transform(crop_volume, 
                                           transform =deskew_transform,
                                           auto_size=True,
                                           linear_interpolation=linear_interpolation)
    else:
    ...
    if debug:
        ...
        if skew_dir=="Y":
            deskewed_crop_no_decon = deskewed_no_decon[:,crop_excess:crop_height+crop_excess,:]
        elif skew_dir=="X":
            deskewed_crop_no_decon = deskewed_no_decon[:,:,crop_excess:crop_width+crop_excess]
...
def get_inverse_affine_transform(original_volume,
                                angle_in_degrees,
    ...
    Args:
        ...
        skew_dir (str): Direction of skew
    ...
    if skew_dir == "Y":
        ...
        deskew_transform = _deskew_x_vol_transform(original_volume,angle_in_degrees,voxel_x,voxel_y,voxel_z)
...
#Get deskew transform where rotation is around centre of "original_volume"
def _deskew_x_vol_transform(original_volume, angle_in_degrees:float = 30, voxel_size_x: float = 1,
              voxel_size_y: float = 1, voxel_size_z: float = 1, scale_factor: float = 1):
